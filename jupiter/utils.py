    ...
    for idx in range(config.max_steps):
        ...
        if config.is_first_stage:
            recv_tensor = torch.zeros(
                tensor_shape["tree_candidates"],
                dtype=tensor_type["tree_candidates"],
            )
        ...
        if config.is_first_stage:
            ...
            recv_tensor = torch.zeros(
                tensor_shape["tree_decoding"],
                dtype=tensor_type["tree_decoding"],
            )
            dist.recv(
                tensor=recv_tensor,
                src=config.pre_rank,
        ...
        if config.is_last_stage:
            ...
            dist.broadcast(
                select_indices_and_new_inputs_ids,
                src=config.total_stage - 1,
            )
        else:
            recv_tensor = torch.zeros(
                tensor_shape["new_token"],
                dtype=tensor_type["new_token"],
            )
            dist.broadcast(
                recv_tensor,
                src=config.total_stage - 1,
...
def outline_based_decoding(model, config, args):
    print(f"[rank {args.rank}] enter outline_based_decoding", flush=True)
    ...
    print(f"[rank {args.rank}] runtime id={id(runtime)} comm_handler id={id(runtime.comm_handler)}", flush=True)

    runtime.jupiter_decoding_pipeline()

    print(f"[rank {args.rank}] before stop_helper_threads runtime id={id(runtime)}", flush=True)
    runtime.comm_handler.stop_helper_threads()
    print(f"[rank {args.rank}] after stop_helper_threads runtime id={id(runtime)}", flush=True)

    print(f"{args.rank}: jupiter_decoding_pipeline done", flush=True)